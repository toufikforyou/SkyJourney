    ...
    <script>
      ...
      function displayTickets(bookings) {
        ...
        bookings.forEach((booking, index) => {
          ...
          const bookingId = booking.bookingId || 'BK' + Math.random().toString(36).substr(2, 9).toUpperCase();
          
          // Check if this booking is already paid (from localStorage)
          const paymentKey = bookingId + '_' + booking.flightNumber + '_' + booking.email;
          const isPaid = localStorage.getItem(paymentKey) === 'true';
          
          const ticketCard = 
            '<div class="ticket-card" data-flight="' + booking.flightNumber + '">' +
            '<div class="ticket-header">' +
            '<div class="booking-info">' +
            '<h3>Booking ID: ' + bookingId + '</h3>' +
            '<p class="flight-number">Flight ' + booking.flightNumber + '</p>' +
            '</div>' +
            '<span class="ticket-status ' + (isPaid ? 'confirmed' : 'pending') + '">' + 
            (isPaid ? 'Confirmed' : 'Pending Payment') + '</span>' +
            '</div>' +
            ...
            '<span class="value">' + booking.seatType + '</span>' +
            '</div>' +
            '<div class="detail-item">' +
            ...
            '<span class="value">' + formatDate(booking.currentTime) + '</span>' +
            '</div>' +
            '</div>' +
            '<div class="ticket-actions">' +
            (isPaid ? 
              '<button class="primary-button paid-btn" disabled>Paid</button>' +
              '<button class="secondary-button cancel-btn" onclick="cancelBooking(\'' + booking.flightNumber + '\', \'' + booking.email + '\', \'' + bookingId + '\')">Cancel Booking</button>' :
              '<button class="primary-button pay-now-btn" onclick="processPayment(\'' + booking.flightNumber + '\', \'' + booking.email + '\')">Pay Now</button>'
            ) +
            '</div>' +
            '</div>';
      ...
      function processPayment(flightNumber, email) {
        ...
        if (targetCard) {
          ...
          setTimeout(() => {
            ...
            const bookingIdElement = targetCard.querySelector('.booking-info h3');
            const bookingId = bookingIdElement.textContent.replace('Booking ID: ', '');
            ...
            actionsDiv.innerHTML = 
              '<button class="primary-button paid-btn" disabled>Paid</button>' +
              '<button class="secondary-button cancel-btn" onclick="cancelBooking(\'' + flightNumber + '\', \'' + email + '\', \'' + bookingId + '\')">Cancel Booking</button>';
